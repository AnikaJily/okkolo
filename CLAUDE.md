...
- **Сборка фронта — локально:** `VITE_STRAPI_URL=https://okkolo-project.ru npm run build` → артефакты в `dist/` → rsync на сервер в `~/apps/web/`. Собирать со старым `http://158.160.128.16` или с дефолтным `http://localhost:1337` (если забыл переменную) НЕЛЬЗЯ — на HTTPS-странице запрос заблокируется как mixed-content и фронт молча свалится на моки. **После деплоя ОБЯЗАТЕЛЬНО проверь живой бандл** (`curl` главной → grep `index-*.js` на `okkolo-project.ru` есть / `158.160`/`localhost` нет) — полная процедура из 3 шагов в `DEPLOY.md` → «Деплой → Фронт». Так уже ломалось (2026-06-30).
