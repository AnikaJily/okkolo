...
> Материал для главы 2 диплома (раздел «Проектирование базы данных»). Диаграмма — на Mermaid
> (нотация Crow's Foot, логический уровень), построена по 17 файлам `schema.json` CMS `okkolo-cms`.
...
Схема как код. Особенность выбранного стека в том, что я не писала SQL-миграции вручную: каждый тип контента описан декларативно в файле `schema.json` (всего 17 описаний, версионируются в Git), а Strapi при старте сам приводит структуру таблиц PostgreSQL к этим описаниям. Тем самым логическая модель и физическая схема всегда синхронизированы, а ER-диаграмма построена непосредственно по этим `schema.json` (имена сущностей и атрибутов на диаграмме соответствуют декларациям; для читаемости опущены вспомогательные alt- и служебные поля).
...
В ходе проектирования я выделила 17 сущностей, которые по назначению объединяются в четыре смысловые группы плюс системную подсистему медиа. По способу хранения они делятся на коллекции (множество записей, 13 шт.) и одиночные типы (single type, ровно одна запись на сайт, 4 шт.).
...
| `event_registrations` — Заявка на мероприятие | Регистрации пользователей на события и заявки «перезвоните» | коллекция |
...
Содержательным ядром предметной области являются три транзакционно значимые сущности — `events`, `products` и порождаемые ими заявки `event_registrations` и `orders`; остальные сущности обслуживают контент сайта и отчётность фонда.
