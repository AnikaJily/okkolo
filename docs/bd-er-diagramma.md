...
Отмечу также, что поля `events.paymentUrl` и `event_registrations.paymentStatus` определены в схеме, но текущая версия фронтенда их не использует: оплата мероприятий принимается на месте, поэтому форма записи не передаёт статус оплаты и не заполняет ссылку на оплату. Поля сохранены в схеме на уровне модели данных и при необходимости позволяют подключить онлайн-оплату без изменения структуры таблиц.
...
2. Мероприятие — Заявка на мероприятие (логическая 1:N, soft-ключ). Связь `EVENT ||..o{ EVENT_REGISTRATION` по смыслу — один-ко-многим: на одно мероприятие может быть подано ноль или несколько заявок, а каждая заявка относится к одному мероприятию. Однако внешнего ключа здесь нет: заявка хранит идентификатор события в строковом поле `eventId` (soft-ключ) и денормализованную копию названия в `eventTitle`. В `eventId` записывается ЧПУ-идентификатор мероприятия — его `slug`, поэтому логически связь соответствует паре `event_registrations.eventId ⇄ events.slug`. Дополнительная тонкость: первичный ключ мероприятия `events.id` — целочисленный, а `eventId` хранит строковый `slug`, поэтому прямого SQL-`JOIN` по паре «id ↔ eventId» by design нет; при необходимости сопоставление выполняется на стороне приложения по `slug`. На диаграмме связь показана пунктиром (неидентифицирующая, не закреплённая FK). Это решение я приняла осознанно — см. 2.3.6.
...
Расцепление от справочников и устойчивость приёма заявок. Отсутствие внешнего ключа делает приём заявок независимым от состояния справочников: запись принимается, даже если соответствующей записи-мероприятия уже не существует (например, мероприятие удалили после регистрации). Это же расцепление даёт свободу в организации каналов приёма: заявки на мастерские я вынесла в отдельный content-type `workshop_applications`, сознательно не связав его внешним ключом ни с `workshop_programs`, ни с какой-либо иной таблицей, — форма «перезвоните» обслуживает все программы целиком (см. 2.3.5, п. 4). Оба решения — soft-ключ у `event_registrations` и автономная таблица `workshop_applications` — следуют одному принципу: каждый канал приёма заявок не зависит от «живого» состояния контентных справочников.
...
Нет, это осознанное решение, продиктованное требованием snapshot-целостности. Заявка и заказ обязаны сохранять состояние данных на момент подачи: при изменении или удалении мероприятия/товара ранее принятая запись не должна искажаться. Поэтому я денормализовала ссылки (`eventId`/`eventTitle`, JSON-`items` с ценами на момент покупки). Дополнительно это расцепляет приём заявок от состояния справочников: запись принимается независимо от того, существует ли ещё запись-мероприятие. По той же причине заявки на мастерские вынесены в отдельную таблицу `workshop_applications`, не связанную внешним ключом с программами. Справочные же сущности при этом нормализованы до 3НФ.
